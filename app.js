...
form.addEventListener('submit', e => {
	...
	if (listItemsArray.length > 0) {
		...
		if (filteredArray.length > 0) {
			msg.textContent = `You already have the weather for ${filteredArray[0].querySelector(".city__name").textContent} ...try adding the country code ex. "US"`;
	...
	fetch(url)
		...
		.then(data => {
			...
			const markup = `
				...
				<div>
					<h2>${Math.round(main.temp)}<sup>°F</sup></h2>
